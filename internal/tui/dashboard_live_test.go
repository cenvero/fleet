...
func dashPress(m model, keys ...string) model {
	for _, k := range keys {
...
func TestDashboardViewFitsTerminalAtEverySize(t *testing.T) {
	...
	for _, sz := range sizes {
		for tab := tabOverview; tab < dashNumTabs; tab++ {
			...
			m = dashPress(m, string(rune('1'+tab)))
			frame := m.View()
...
func TestServerSortCyclesAndReverses(t *testing.T) {
	...
	m = dashPress(m, "o") // status
	m = dashPress(m, "o") // cpu (hottest first)
...
func TestServerFilterIsIncrementalAndCombinesTerms(t *testing.T) {
	...
	m = dashPress(m, "backspace", "backspace", "backspace", "backspace", "backspace", "backspace", "backspace")
	if len(m.views.servers) != 90 {
...
func TestDashboardUpdateAtSeveralSizes(t *testing.T) {
	for _, sz := range [][2]int{{80, 24}, {120, 40}, {200, 60}} {
		...
		for tab := 1; tab <= 6; tab++ {
			m = dashPress(m, fmt.Sprint(tab), "j", "j", "pgdown", "k", "enter", "esc", "?", "esc")
...
func dashRunCmd(t *testing.T, cmd tea.Cmd) tea.Msg {
	t.Helper()
	if cmd == nil {
...
func TestFailedActionReportsCLIError(t *testing.T) {
	...
	next, cmd := m.Update(keyMsg("y"))
	m = next.(model)
	next, _ = m.Update(dashRunCmd(t, cmd))
	m = next.(model)
...
func TestInteractiveActionsSuspendTheTUI(t *testing.T) {
	...
	m = dashPress(m, "2", "j") // node-001
	m = dashPress(m, "s", "f")
	m = dashPress(m, "3", "L") // first service row
	if len(got) != 3 {
