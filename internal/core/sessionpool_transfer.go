...
func (tc *transferChannels) grow(want int) {
	...
	if missing := want - have - len(added); missing > 0 {
		...
		for i := range missing {
			...
			go func(i int) {
				defer wg.Done()
				// Bounded like the pool's own channel opens, so a half-dead
				// connection cannot stall the transfer's setup.
				child, err := base.session().OpenChannelSessionContext(context.Background(), pooledChannelOpenTimeout)
				if err != nil {
					return // fewer streams than asked for is fine
				}
				// Tie the lease to the connection it rides on, so release never
				// hands this channel to a replacement connection.
				opened[i] = &lease{pool: base.pool, name: base.name, sess: child, fromNew: true, entry: base.entry}
			}(i)
		}
		wg.Wait()
...
func (tc *transferChannels) retire(slot int, l *lease) {
	...
	fresh, err := sess.OpenChannelSessionContext(context.Background(), pooledChannelOpenTimeout)
	if err != nil {
	...
	if tc.leases[slot] == nil {
		tc.leases[slot] = &lease{pool: l.pool, name: l.name, sess: fresh, fromNew: true, entry: l.entry}
		fresh = nil
	}
	tc.mu.Unlock()
	if fresh != nil {
...
func (sp *sessionPool) isCurrentConn(serverName string, client *ssh.Client) bool {
	...
	return ok && !entry.dead && entry.root != nil && entry.root.Client == client
}

// evictConn drops serverName's pooled connection only if it is still backed by
// client. It goes through retire, so the entry is marked dead, its keepalive
// stopped and its idle channels closed exactly as for any other retirement.
func (sp *sessionPool) evictConn(serverName string, client *ssh.Client) {
	...
	sp.mu.Lock()
	entry, ok := sp.entries[serverName]
	matches := ok && entry.root != nil && entry.root.Client == client
	sp.mu.Unlock()
	if matches {
